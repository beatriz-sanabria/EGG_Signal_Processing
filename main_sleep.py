...
import xlsxwriter
from close_open_eyes import init_marks, sleep_segment
from signal_filtering import sleep_segment_filtering
import mne
import constant
...
for route in routes:
    ...
    sleepsegment = sleep_segment(events_eeg, raw,
                                 duration_bad_marks)  # Call function that recover the segment of sleep
    filt_sleep_segment = sleep_segment_filtering(sleepsegment)

    if routes[0] == eeg:
        finalraw = filt_sleep_segment
    else:
        isGroupAnalysis = True
        finalraw = mne.io.concatenate_raws([finalraw, filt_sleep_segment])

    filt_sleep_segment.plot(duration=constant.SEC, start=constant.SEC_START, color='darkblue',
                            n_channels=constant.NUM_CHANNELS,
                            scalings=constant.EEG_AMPLITUDE, block=True)
    # PSD calculation
    power_psd, amp_psd, freqs, psd = psd_calculation(filt_sleep_segment)

    # Frequency calculation
    frequencies = freq_per_channel(filt_sleep_segment, power_psd, freqs)
    ...
    amplitudes = amp_per_channel(filt_sleep_segment, amp_psd)
    front_amp, central_amp, posterior_amp, right_amp, left_amp, medium_amp = amplitude_per_area(amplitudes)

    # Power calculation
    power = power_per_channel(freqs, power_psd, filt_sleep_segment)
    ...
    worksheet1 = workbook.add_worksheet("Sleep")
    ...
    filt_sleep_segment.plot_psd(fmin=0.5, fmax=45, tmin=30, tmax=280, proj=False, picks=None, n_fft=2048, dB=False,
                                estimate='amplitude')
    filt_sleep_segment.plot_psd(fmin=0.5, fmax=45, tmin=30, tmax=280, proj=False, picks=None, n_fft=2048, dB=False,
                                estimate='power')

    plotting_topographies(power_psd, freqs, filt_sleep_segment, min_delta, max_delta, min_theta, max_theta, min_alpha,
...
if isGroupAnalysis:
    ...
    # Absolute power per areas calculation
    front_d, front_t, front_a, front_b_1, front_b_2, front_g, central_d, central_t, central_a, central_b_1, \
    central_b_2, central_g, posterior_d, posterior_t, posterior_a, posterior_b_1, posterior_b_2, posterior_g, right_d, \
    right_t, right_a, right_b_1, right_b_2, right_g, left_d, left_t, left_a, left_b_1, left_b_2, left_g, medium_d, \
    medium_t, medium_a, medium_b_1, medium_b_2, medium_g = power_per_area(power)
    ...
    rel_left_t, rel_left_a, rel_left_b_1, rel_left_b_2, rel_left_g, rel_medium_d, rel_medium_t, rel_medium_a, \
    rel_medium_b_1, rel_medium_b_2, rel_medium_g = relative_power_per_area(front_d, front_t, front_a, front_b_1,
                                                                           front_b_2, front_g, central_d, central_t,
                                                                           central_a, central_b_1, central_b_2,
                                                                           central_g, posterior_d, posterior_t,
                                                                           posterior_a, posterior_b_1, posterior_b_2,
                                                                           posterior_g, right_d, right_t, right_a,
                                                                           right_b_1, right_b_2, right_g, left_d,
                                                                           left_t, left_a, left_b_1, left_b_2,
                                                                           left_g, medium_d, medium_t, medium_a,
                                                                           medium_b_1, medium_b_2, medium_g)

    workbook = xlsxwriter.Workbook(file_name)
    worksheet1 = workbook.add_worksheet("SleepGroup")
    ...
    data = (['Name', name], ['Age', age], ['Laterality', laterality], ['Group', group], ['State', state],
            ...
            ['Rel Power Gamma', rel_gamma], ['Front Abs Power Delta', front_d], ['Front Abs Power Theta', front_t],
            ['Front Abs Power Alpha', front_a], ['Front Abs Power Beta 1', front_b_1],
            ['Front Abs Power Beta 2', front_b_2], ['Central Abs Power Theta', central_t],
            ['Central Abs Power Alpha', central_a], ['Central Abs Power Beta 1', central_b_1],
            ['Central Abs Power Beta 2', central_b_2], ['Central Abs Power Gamma', central_g],
            ['Posterior Abs Power Delta', posterior_d], ['Posterior Abs Power Theta', posterior_t],
            ['Posterior Abs Power Alpha', posterior_a], ['Posterior Abs Power Beta 1', posterior_b_1],
            ['Posterior Abs Power Beta 2', posterior_b_2], ['Posterior Abs Power Gamma', posterior_g],
            ['Right Abs Power Delta', right_d], ['Right Abs Power Theta', right_t],
            ['Right Abs Power Alpha', right_a], ['Right Abs Power Beta 1', right_b_1],
            ['Right Abs Power Beta 2', right_b_2], ['Right Abs Power Gamma', right_g], ['Left Abs Power Delta', left_d],
            ['Left Abs Power Theta', left_t], ['Left Abs Power Alpha', left_a], ['Left Abs Power Beta 1', left_b_1],
            ['Left Abs Power Beta 2', left_b_2], ['Left Abs Power Gamma', left_g], ['Medium Abs Power Delta', medium_d],
            ['Medium Abs Power Theta', medium_t], ['Medium Abs Power Alpha', medium_a],
